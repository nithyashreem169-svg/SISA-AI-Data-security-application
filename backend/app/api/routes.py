...
async def ai_analysis(
    ...
    try:
        ...
        if enable_remediation and findings:
            ...
            if remediations:
                response['remediation'] = remediations
        
        # AI-Recommended Actions (NEW)
        if findings:
            logger.info("Generating AI-recommended actions...")
            risk_score = int(request_data.get('risk_score', 0))
            recommended_actions = ai_analyzer.generate_recommended_actions(findings, risk_score)
            if recommended_actions:
                response['recommended_actions'] = recommended_actions
